...
bool plain::a_open_in(FILE *&ios) {
  ...
  if (found == false) {
    const std::string &absolute_path =
        input_path + name_of_file; // look relative to the the input directory;
                                   // user macro definitions will be found there
...
bool plain::a_open_out(FILE *&ios) {
...
bool plain::b_open_out(FILE *&ios) {
  std::string absolute_path = output_path + name_of_file;
  strncpy(name_of_file, absolute_path.c_str(), file_name_size - 1);
...
bool plain::w_open_out(FILE *&ios) {
  std::string absolute_path = output_path + name_of_file;
  strncpy(name_of_file, absolute_path.c_str(), file_name_size - 1);
...
void plain::typeset(const std::string &filename, const std::string &result,
                    const std::string &search_dir,
                    const std::string &working_dir, const std::string &output) {

...
void tex::print_char(ASCII_code s) {
...
void tex::print_nl(str_number s) {
...
void tex::print_esc(str_number s) {
...
void tex::print_the_digs(uint8_t k) {
  while (k > 0) {
    --k;
    if (dig[k] < 10)
      print_char(dig[k] + 48);
...
void tex::print_file_name(integer n, integer a, integer e) {
  slow_print(a);
  slow_print(n);
  slow_print(e);
}

void tex::print_size(integer s) {
  if (!s) {
    print_esc(412);
    return;
  }
...
void tex::print_write_whatsit(str_number s, halfword p) {
  print_esc(s);
...
void tex::fatal_error(str_number s) {
  normalize_selector();
  print_nl(262);
  print(287);
  help_ptr = 1;
...
void tex::overflow(str_number s, integer n) {
  normalize_selector();
  print_nl(262);
  print(288);
  print(s);
...
void tex::confusion(str_number s) {
  normalize_selector();
  if (history < 2) {
    print_nl(262);
    print(291);
...
void tex::print_two(integer n) {
  n = abs(n) % 100;
  print_char((n / 10) + 48);
  print_char((n % 10) + 48);
}

void tex::print_hex(integer n) {
  char k = 0;
  print_char(34);
  do {
    dig[k] = n & 15;
...
void tex::print_roman_int(integer n) {
  pool_pointer k;
...
void tex::pause_for_instructions() {
  if (!OK_to_interrupt)
    return;
  interaction = 3;
...
scaled tex::round_decimals(small_number k) {
  integer a = 0;
  while (k > 0) {
    --k;
    a = (a + (dig[k] * 131072)) / 10;
...
void tex::print_scaled(scaled s) {
  scaled delta = 10;
  if (s < 0) {
    print_char(45);
    s = -s;
...
scaled tex::mult_and_add(integer n, scaled x, scaled y, scaled max_answer) {
  if (n < 0) {
    x = -x;
    n = -n;
  }
...
halfword tex::badness(scaled t, scaled s) {
  integer r;
  if (!t) {
    return 0;
  } else if (s <= 0) {
...
void tex::show_token_list(integer p, integer q, integer l) {
  integer m, c;
...
void tex::flush_list(halfword p) {
  halfword q, r;
  if (p == (-1073741824))
    return;
  r = p;
...
halfword tex::get_node(integer s) {
  halfword result, p, q;
  integer r, t;
_L20:
  p = rover;
...
void tex::free_node(halfword p, halfword s) {
  halfword q;
...
halfword tex::new_ligature(quarterword f, quarterword c, halfword q) {
...
halfword tex::new_lig_item(quarterword c) {
  halfword p = get_node(2);
...
halfword tex::new_math(scaled w, small_number s) {
  halfword p = get_node(2);
...
halfword tex::new_spec(halfword p) {
...
halfword tex::new_param_glue(small_number n) {
  halfword q;
  halfword p = get_node(2);
...
halfword tex::new_glue(halfword q) {
  halfword p = get_node(2);
  mem[p - mem_min].hh.U2.b0 = 10;
  mem[p - mem_min].hh.U2.b1 = 0;
  mem[p - mem_min + 1].hh.rh = -1073741824;
...
halfword tex::new_skip_param(small_number n) {
  halfword p;
...
halfword tex::new_kern(scaled w) {
  halfword p = get_node(2);
...
halfword tex::new_penalty(integer m) {
  halfword p = get_node(2);
...
void tex::short_display(integer p) {
  integer n;
  while (p > mem_min) {
    if (p >= hi_mem_min) {
      if (p <= mem_end) {
...
void tex::print_font_and_char(integer p) {
  if (p > mem_end) {
    print_esc(309);
    return;
  }
...
void tex::print_rule_dimen(scaled d) {
  if (d == (-1073741824))
    print_char(42);
  else
    print_scaled(d);
...
void tex::print_glue(scaled d, integer order, str_number s) {
  print_scaled(d);
  if (((unsigned)order) > 3) {
    print(310);
    return;
...
void tex::print_spec(integer p, str_number s) {
...
void tex::print_fam_and_char(halfword p) {
  print_esc(464);
...
void tex::print_delimiter(halfword p) {
  integer a = (mem[p - mem_min].qqqq.b0 * 256) + mem[p - mem_min].qqqq.b1;
  a = (a * 4096) + (mem[p - mem_min].qqqq.b2 * 256) + mem[p - mem_min].qqqq.b3;
  if (a < 0)
    print_int(a);
  else
    print_hex(a);
...
void tex::print_subsidiary_data(halfword p, ASCII_code c) {
...
void tex::print_style(integer c) {
  switch (c / 2) {
  case 0:
    print_esc(860);
    break;
...
void tex::print_skip_param(integer n) {
  switch (n) {
  case 0:
    print_esc(376);
    break;
...
void tex::show_node_list(integer p) {
  integer n = 0;
  double g;
  if (pool_ptr - str_start[str_ptr] > depth_threshold) {
    if (p > (-1073741824))
      ...
      case 13:
        ...
        if (mem[p - mem_min].hh.U2.b0 == 13) {
          ...
          if (mem[p - mem_min + 6].int_) {
            print(323);
            print_glue(mem[p - mem_min + 6].int_, mem[p - mem_min + 5].hh.U2.b1,
                       0);
          }
          if (mem[p - mem_min + 4].int_) {
            print(324);
            print_glue(mem[p - mem_min + 4].int_, mem[p - mem_min + 5].hh.U2.b0,
                       0);
          }
        } else {
...
void tex::delete_token_ref(halfword p) {
  if (mem[p - mem_min].hh.lh == (-1073741824))
    flush_list(p);
  else
    --mem[p - mem_min].hh.lh;
...
void tex::delete_glue_ref(halfword p) {
  if (mem[p - mem_min].hh.rh == (-1073741824))
    free_node(p, 4);
  else
    --mem[p - mem_min].hh.rh;
...
void tex::flush_node_list(halfword p) {
  halfword q;
  while (p != (-1073741824)) {
...
halfword tex::copy_node_list(halfword p) {
  halfword r;
  char words;
  ...
  while (p != (-1073741824)) {
    ...
    if (p >= hi_mem_min) {
      ...
      case 13:
        ...
        mem[r - mem_min + 5].hh.rh = copy_node_list(mem[p - mem_min + 5].hh.rh);
        words = 5;
        break;
      case 2:
        r = get_node(4);
      ...
      case 3:
        ...
        mem[r - mem_min + 4].hh.lh = copy_node_list(mem[p - mem_min + 4].hh.lh);
        words = 4;
        break;
      case 8:
        switch (mem[p - mem_min].hh.U2.b1) {
      ...
      case 10:
        ...
        mem[r - mem_min + 1].hh.rh = copy_node_list(mem[p - mem_min + 1].hh.rh);
        break;
      case 11:
      case 9:
      case 12:
      ...
      case 7:
        r = get_node(2);
        mem[r - mem_min + 1].hh.lh = copy_node_list(mem[p - mem_min + 1].hh.lh);
        mem[r - mem_min + 1].hh.rh = copy_node_list(mem[p - mem_min + 1].hh.rh);
        break;
      case 4:
        r = get_node(2);
...
void tex::print_param(integer n) {
  switch (n) {
  case 0:
    print_esc(420);
    break;
...
void tex::print_length_param(integer n) {
  switch (n) {
  case 0:
    print_esc(478);
    break;
...
void tex::print_cmd_chr(quarterword cmd, halfword chr_code) {
  switch (cmd) {
  case 1:
    print(557);
    print(chr_code);
...
halfword tex::id_lookup(integer j, integer l) {
  integer d;
  halfword p, k;
  integer h = buffer[j];
  for (k = j + 1; k <= (j + l - 1); ++k) {
  ...
  while (true) {
    if (hash[p - 514].rh > 0) {
      if (str_start[hash[p - 514].rh + 1] - str_start[hash[p - 514].rh] == l) {
...
void tex::primitive(str_number s, quarterword c, halfword o) {
  pool_pointer k;
  small_number j, l;
  if (s < 256) {
    cur_val = s + 257;
...
void tex::new_save_level(group_code c) {
...
void tex::eq_destroy(memory_word w) {
  halfword q;
  switch (w.hh.U2.b0) {
  case 111:
  case 112:
...
void tex::eq_save(halfword p, quarterword l) {
  if (save_ptr > max_save_stack) {
    max_save_stack = save_ptr;
    if (max_save_stack > (save_size - 6))
      overflow(541, save_size);
...
void tex::eq_define(halfword p, quarterword t, halfword e) {
...
void tex::eq_word_define(halfword p, integer w) {
...
void tex::geq_define(halfword p, quarterword t, halfword e) {
  eq_destroy(eqtb[p]);
  eqtb[p].hh.U2.b1 = 1;
  eqtb[p].hh.U2.b0 = t;
  eqtb[p].hh.rh = e;
...
void tex::geq_word_define(halfword p, integer w) {
  eqtb[p].int_ = w;
  xeq_level[p - 12163] = 1;
}

void tex::save_for_after(halfword t) {
  if (cur_level <= 1)
    return;
  if (save_ptr > max_save_stack) {
    max_save_stack = save_ptr;
...
void tex::token_show(halfword p) {
  if (p != (-1073741824))
      ...
      if ((base_ptr == input_ptr) || cur_input.state_field ||
        ...
        if (cur_input.state_field) {
          ...
          if (cur_input.index_field < 5)
            show_token_list(cur_input.start_field, cur_input.loc_field, 100000);
          else
...
void tex::begin_token_list(halfword p, quarterword t) {
...
void tex::clear_for_error_prompt() {
  while (cur_input.state_field && (!cur_input.name_field) && (input_ptr > 0) &&
...
halfword tex::str_toks(pool_pointer b) {
...
void tex::read_toks(integer n, halfword r) {
  halfword p, q;
  integer s;
  small_number m;
  scanner_status = 2;
...
void tex::change_if_limit(small_number l, halfword p) {
  halfword q;
  if (p == cond_ptr) {
    if_limit = l;
  } else {
...
void tex::pack_file_name(str_number n, str_number a, str_number e) {
  integer k = 0;
  ASCII_code c;
  pool_pointer j, N1;
...
void tex::pack_buffered_name(small_number n, integer a, integer b) {
  integer k = 0, j;
  ASCII_code c;
...
str_number tex::make_name_string() {
  int k, N;
...
void tex::pack_job_name(str_number s) {
  cur_area = 338;
  cur_ext = s;
  cur_name = job_name;
...
void tex::prompt_file_name(str_number s, str_number e) {
  int k;
  if (s == 786) {
    print_nl(262);
    print(787);
...
internal_font_number tex::read_font_info(halfword u, str_number nom,
  ...
  lf = (lf * 256) + readU8(tfm_file, tfm_file_mode, &tfm_file_value);
  loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
  lh = readU8(tfm_file, tfm_file_mode, &tfm_file_value);
  if (lh > 127)
    goto _L11;
  loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
  lh = (lh * 256) + readU8(tfm_file, tfm_file_mode, &tfm_file_value);
  loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
  bc = readU8(tfm_file, tfm_file_mode, &tfm_file_value);
  if (bc > 127)
    goto _L11;
  loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
  bc = (bc * 256) + readU8(tfm_file, tfm_file_mode, &tfm_file_value);
  loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
  ec = readU8(tfm_file, tfm_file_mode, &tfm_file_value);
  if (ec > 127)
    goto _L11;
  loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
  ec = (ec * 256) + readU8(tfm_file, tfm_file_mode, &tfm_file_value);
  if ((bc > (ec + 1)) || (ec > 255))
    goto _L11;
  if (bc > 255) {
    bc = 1;
  ...
  if (nw > 127)
    goto _L11;
  loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
  nw = (nw * 256) + readU8(tfm_file, tfm_file_mode, &tfm_file_value);
  loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
  nh = readU8(tfm_file, tfm_file_mode, &tfm_file_value);
  if (nh > 127)
    goto _L11;
  loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
  nh = (nh * 256) + readU8(tfm_file, tfm_file_mode, &tfm_file_value);
  loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
  nd = readU8(tfm_file, tfm_file_mode, &tfm_file_value);
  if (nd > 127)
    goto _L11;
  loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
  nd = (nd * 256) + readU8(tfm_file, tfm_file_mode, &tfm_file_value);
  loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
  ni = readU8(tfm_file, tfm_file_mode, &tfm_file_value);
  if (ni > 127)
    goto _L11;
  loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
  ni = (ni * 256) + readU8(tfm_file, tfm_file_mode, &tfm_file_value);
  loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
  nl = readU8(tfm_file, tfm_file_mode, &tfm_file_value);
  if (nl > 127)
    goto _L11;
  loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
  nl = (nl * 256) + readU8(tfm_file, tfm_file_mode, &tfm_file_value);
  loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
  nk = readU8(tfm_file, tfm_file_mode, &tfm_file_value);
  if (nk > 127)
    goto _L11;
  loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
  nk = (nk * 256) + readU8(tfm_file, tfm_file_mode, &tfm_file_value);
  loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
  ne = readU8(tfm_file, tfm_file_mode, &tfm_file_value);
  if (ne > 127)
    goto _L11;
  loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
  ne = (ne * 256) + readU8(tfm_file, tfm_file_mode, &tfm_file_value);
  loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
  np = readU8(tfm_file, tfm_file_mode, &tfm_file_value);
  if (np > 127)
    goto _L11;
  loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
  np = (np * 256) + readU8(tfm_file, tfm_file_mode, &tfm_file_value);
  ...
  if (z > 127)
    goto _L11;
  loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
  z = (z * 256) + readU8(tfm_file, tfm_file_mode, &tfm_file_value);
  loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
  z = (z * 256) + readU8(tfm_file, tfm_file_mode, &tfm_file_value);
  loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
  z = (z * 16) + (readU8(tfm_file, tfm_file_mode, &tfm_file_value) / 16);
  if (z < 65536)
    goto _L11;
  while (lh > 2) {
    loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
  ...
  beta = 256 / alpha;
  alpha *= z;
  for (N = fnt_infos.lig_kern_base[f], k = fnt_infos.width_base[f];
       k <= (N - 1); ++k) {
    loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
  ...
  if (font_info[fnt_infos.italic_base[f]].int_)
    goto _L11;
  if (nl > 0) {
    for (N = fnt_infos.kern_base[f] + 32767, k = fnt_infos.lig_kern_base[f];
         k <= N; ++k) {
      loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
      a = readU8(tfm_file, tfm_file_mode, &tfm_file_value);
      qw.b0 = a;
  ...
  for (N = fnt_infos.exten_base[f], k = fnt_infos.kern_base[f] + 32768;
       k <= (N - 1); ++k) {
    loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
    fread(&a, sizeof(uint8_t), 1, tfm_file);
    fread(&b, sizeof(uint8_t), 1, tfm_file);
  ...
  for (N = fnt_infos.param_base[f], k = fnt_infos.exten_base[f]; k <= (N - 1);
       ++k) {
    loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
    a = readU8(tfm_file, tfm_file_mode, &tfm_file_value);
    qw.b0 = a;
  ...
  for (k = 1; k <= np; ++k) {
    if (k == 1) {
      ...
      if (sw > 127)
        sw -= 256;
      loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
      sw = (sw * 256) + readU8(tfm_file, tfm_file_mode, &tfm_file_value);
      loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
      sw = (sw * 256) + readU8(tfm_file, tfm_file_mode, &tfm_file_value);
      loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
      font_info[fnt_infos.param_base[f]].int_ =
          (sw * 16) + (readU8(tfm_file, tfm_file_mode, &tfm_file_value) / 16);
    } else {
      loadU8(tfm_file, tfm_file_mode, &tfm_file_value);
      fread(&a, sizeof(uint8_t), 1, tfm_file);
      fread(&b, sizeof(uint8_t), 1, tfm_file);
...
void tex::char_warning(internal_font_number f, uint8_t c) {
...
halfword tex::new_character(internal_font_number f, uint8_t c) {
  halfword result, p;
...
void tex::write_dvi(dvi_index a, dvi_index b) {
  dvi_index k;
  for (k = a; k <= b; ++k)
    fprintf(dvi_mgr.dvi_file, "%c", dvi_mgr.dvi_buf[k]);
}

void tex::dvi_swap() {
  if (dvi_mgr.dvi_limit == dvi_buf_size) {
    write_dvi(0, dvi_mgr.half_buf - 1);
    dvi_mgr.dvi_limit = dvi_mgr.half_buf;
    dvi_mgr.dvi_offset += dvi_buf_size;
    dvi_mgr.dvi_ptr = 0;
  } else {
    write_dvi(dvi_mgr.half_buf, dvi_buf_size - 1);
    dvi_mgr.dvi_limit = dvi_buf_size;
  }
  dvi_mgr.dvi_gone += dvi_mgr.half_buf;
}

void tex::dvi_four(integer x) {
  if (x >= 0) {
    dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = x / 16777216;
    ++dvi_mgr.dvi_ptr;
    if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
      dvi_swap();
  } else {
    x += 1073741824;
    x += 1073741824;
    dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = (x / 16777216) + 128;
    ++dvi_mgr.dvi_ptr;
    if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
      dvi_swap();
  }
  x &= 16777215;
  dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = x / 65536;
  ++dvi_mgr.dvi_ptr;
  if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
    dvi_swap();
  x &= 65535;
  dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = x / 256;
  ++dvi_mgr.dvi_ptr;
  if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
    dvi_swap();
  dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = x & 255;
  ++dvi_mgr.dvi_ptr;
  if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
    dvi_swap();
}

void tex::dvi_pop(integer l) {
  if ((l == (dvi_mgr.dvi_offset + dvi_mgr.dvi_ptr)) && (dvi_mgr.dvi_ptr > 0)) {
    --dvi_mgr.dvi_ptr;
    return;
  }
  dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = 142;
  ++dvi_mgr.dvi_ptr;
  if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
    dvi_swap();
}

void tex::dvi_font_def(internal_font_number f) {
  pool_pointer k, N;
  dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = 243;
  ++dvi_mgr.dvi_ptr;
  if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
    dvi_swap();
  dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = f - 1;
  ++dvi_mgr.dvi_ptr;
  if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
    dvi_swap();
  dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = fnt_infos.font_check[f].b0;
  ++dvi_mgr.dvi_ptr;
  if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
    dvi_swap();
  dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = fnt_infos.font_check[f].b1;
  ++dvi_mgr.dvi_ptr;
  if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
    dvi_swap();
  dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = fnt_infos.font_check[f].b2;
  ++dvi_mgr.dvi_ptr;
  if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
    dvi_swap();
  dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = fnt_infos.font_check[f].b3;
  ++dvi_mgr.dvi_ptr;
  if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
    dvi_swap();
  dvi_four(fnt_infos.font_size[f]);
  dvi_four(fnt_infos.font_dsize[f]);
  dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] =
      str_start[fnt_infos.font_area[f] + 1] - str_start[fnt_infos.font_area[f]];
  ++dvi_mgr.dvi_ptr;
  if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
    dvi_swap();
  dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] =
      str_start[fnt_infos.font_name[f] + 1] - str_start[fnt_infos.font_name[f]];
  ++dvi_mgr.dvi_ptr;
  if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
    dvi_swap();
  for (N = str_start[fnt_infos.font_area[f] + 1],
      k = str_start[fnt_infos.font_area[f]];
       k <= (N - 1); ++k) {
    dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = str_pool[k];
    ++dvi_mgr.dvi_ptr;
    if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
      dvi_swap();
  }
  for (N = str_start[fnt_infos.font_name[f] + 1],
      k = str_start[fnt_infos.font_name[f]];
       k <= (N - 1); ++k) {
    dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = str_pool[k];
    ++dvi_mgr.dvi_ptr;
    if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
...
void tex::movement(scaled w, uint8_t o) {
  small_number mstate = 0;
  halfword p;
  integer k;
  halfword q = get_node(3);
  mem[q - mem_min + 1].int_ = w;
  mem[q - mem_min + 2].int_ = dvi_mgr.dvi_offset + dvi_mgr.dvi_ptr;
  if (o == 157) {
  ...
  while (p != (-1073741824)) {
    if (mem[p - mem_min + 1].int_ == w) {
      ...
      case 15:
      case 16:
        if (mem[p - mem_min + 2].int_ < dvi_mgr.dvi_gone) {
          goto _L45;
        } else {
          k = mem[p - mem_min + 2].int_ - dvi_mgr.dvi_offset;
          if (k < 0)
            k += dvi_buf_size;
          dvi_mgr.dvi_buf[k] += 5;
...
_L45:
  ...
  if (abs(w) >= 8388608) {
    dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = o + 3;
    ++dvi_mgr.dvi_ptr;
    if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
      dvi_swap();
    dvi_four(w);
  ...
  if (abs(w) >= 32768) {
    dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = o + 2;
    ++dvi_mgr.dvi_ptr;
    if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
      dvi_swap();
    if (w < 0)
      w += 16777216;
    dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = w / 65536;
    ++dvi_mgr.dvi_ptr;
    if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
      dvi_swap();
    w &= 65535;
  ...
  if (abs(w) >= 128) {
    dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = o + 1;
    ++dvi_mgr.dvi_ptr;
    if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
      dvi_swap();
    if (w < 0)
  ...
  dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = o;
  ++dvi_mgr.dvi_ptr;
  if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
    dvi_swap();
  if (w < 0)
  ...
  goto _L1;
_L2:
  dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = w / 256;
  ++dvi_mgr.dvi_ptr;
  if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
    dvi_swap();
_L1:
  dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = w & 255;
  ++dvi_mgr.dvi_ptr;
  if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
    dvi_swap();
  goto _L10;
_L40:
  ...
  if (mem[q - mem_min].hh.lh == 1) {
    dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = o + 4;
    ++dvi_mgr.dvi_ptr;
    if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
      dvi_swap();
    ...
    dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = o + 9;
    ++dvi_mgr.dvi_ptr;
    if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
      dvi_swap();
    while (mem[q - mem_min].hh.rh != p) {
...
void tex::prune_movements(integer l) {
  halfword p;
...
void tex::special_out(halfword p) {
  char old_setting;
  pool_pointer k, N;
  if (cur_h != dvi_mgr.dvi_h) {
    movement(cur_h - dvi_mgr.dvi_h, 143);
    dvi_mgr.dvi_h = cur_h;
  }
  if (cur_v != dvi_mgr.dvi_v) {
    movement(cur_v - dvi_mgr.dvi_v, 157);
    dvi_mgr.dvi_v = cur_v;
  }
  old_setting = selector;
  selector = 21;
  show_token_list(mem[mem[p - mem_min + 1].hh.rh - mem_min].hh.rh, -1073741824,
                  pool_size - pool_ptr);
  selector = old_setting;
  if (pool_ptr + 1 > pool_size)
    overflow(257, pool_size - init_pool_ptr);
  if (pool_ptr - str_start[str_ptr] < 256) {
    dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = 239;
    ++dvi_mgr.dvi_ptr;
    if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
      dvi_swap();
    dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = pool_ptr - str_start[str_ptr];
    ++dvi_mgr.dvi_ptr;
    if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
      dvi_swap();
  } else {
    dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = 242;
    ++dvi_mgr.dvi_ptr;
    if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
      dvi_swap();
...
void tex::hlist_out() {
  ...
  ++cur_s;
  if (cur_s > 0) {
    dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = 141;
    ++dvi_mgr.dvi_ptr;
    if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
      dvi_swap();
  }
  if (cur_s > max_push)
    max_push = cur_s;
  save_loc = dvi_mgr.dvi_offset + dvi_mgr.dvi_ptr;
  base_line = cur_v;
  left_edge = cur_h;
  while (p != (-1073741824)) {
  _L21:
    if (p >= hi_mem_min) {
      ...
      do {
        ...
        if (f != dvi_mgr.dvi_f) {
          ...
          if (f <= 64) {
            dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = f + 170;
            ++dvi_mgr.dvi_ptr;
            if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
              dvi_swap();
          } else {
            dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = 235;
            ++dvi_mgr.dvi_ptr;
            if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
              dvi_swap();
            dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = f - 1;
        ...
        if (c >= 128) {
          dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = 128;
          ++dvi_mgr.dvi_ptr;
          if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
            dvi_swap();
        }
        dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = c;
        ++dvi_mgr.dvi_ptr;
        if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
          dvi_swap();
        cur_h += font_info[fnt_infos.width_base[f] +
                           font_info[fnt_infos.char_base[f] + c].qqqq.b0]
                     .int_;
        p = mem[p - mem_min].hh.rh;
      } while (p >= hi_mem_min);
      dvi_mgr.dvi_h = cur_h;
    ...
    case 1:
      if (mem[p - mem_min + 5].hh.rh == (-1073741824)) {
        ...
        save_h = dvi_mgr.dvi_h;
        save_v = dvi_mgr.dvi_v;
        ...
        if (mem[p - mem_min].hh.U2.b0 == 1)
          ...
          hlist_out();
        dvi_mgr.dvi_h = save_h;
        dvi_mgr.dvi_v = save_v;
  ...
  _L14:
    ...
    if ((rule_ht > 0) && (rule_wd > 0)) {
      ...
      dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = 132;
      ++dvi_mgr.dvi_ptr;
      if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
        dvi_swap();
      dvi_four(rule_ht);
  ...
  while (p != (-1073741824)) {
        ...
        if (mem[p - mem_min].hh.U2.b1 >= 100) {
          ...
          if ((leader_ht > 0) && (rule_ht > 0)) {
            ...
            while (cur_v + leader_ht <= edge) {
              ...
              cur_h = left_edge;
              cur_v =
                  save_v - mem[leader_box - mem_min + 3].int_ + leader_ht + lx;
            }
            cur_v = edge - 10;
            goto _L15;
          }
    ...
    _L14:
      ...
      if ((rule_ht > 0) && (rule_wd > 0)) {
        ...
        dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = 137;
        ++dvi_mgr.dvi_ptr;
        if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
          dvi_swap();
        dvi_four(rule_ht);
...
void tex::ship_out(halfword p) {
  ...
  if (mem[p - mem_min + 3].int_ + mem[p - mem_min + 2].int_ + eqtb[12749].int_ >
      max_v)
  ...
  if (!dvi_mgr.total_pages) {
    dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = 247;
    ++dvi_mgr.dvi_ptr;
    if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
      dvi_swap();
    dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = 2;
    ++dvi_mgr.dvi_ptr;
    if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
      dvi_swap();
    dvi_four(25400000);
    ...
    for (N = pool_ptr, s = str_start[str_ptr]; s <= (N - 1); ++s) {
      dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = str_pool[s];
      ++dvi_mgr.dvi_ptr;
      if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
        dvi_swap();
    }
  ...
  page_loc = dvi_mgr.dvi_offset + dvi_mgr.dvi_ptr;
  dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = 139;
  ++dvi_mgr.dvi_ptr;
  if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
    dvi_swap();
  for (k = 0; k <= 9; ++k)
  ...
  if (mem[p - mem_min].hh.U2.b0 == 1)
    ...
    hlist_out();
  dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = 140;
  ++dvi_mgr.dvi_ptr;
  if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
    dvi_swap();
  ++dvi_mgr.total_pages;
...
halfword tex::hpack(halfword p, scaled w, small_number m) {
  halfword r, q, g;
...
halfword tex::vpackage(halfword p, scaled h, small_number m, scaled l) {
  halfword r, g;
...
void tex::append_to_vlist(halfword b) {
  scaled d;
  halfword p;
...
halfword tex::new_style(small_number s) {
...
void tex::show_info() { show_node_list(mem[temp_ptr - mem_min].hh.lh); }

halfword tex::fraction_rule(scaled t) {
  halfword p = new_rule();
...
halfword tex::overbar(halfword b, scaled k, scaled t) {
  halfword q;
...
halfword tex::char_box(internal_font_number f, quarterword c) {
  halfword p;
  four_quarters q = font_info[fnt_infos.char_base[f] + c].qqqq;
  uint8_t hd = q.b1;
  halfword b = new_null_box();
  mem[b - mem_min + 1].int_ =
      font_info[fnt_infos.width_base[f] + q.b0].int_ +
      font_info[fnt_infos.italic_base[f] + (q.b2 / 4)].int_;
  mem[b - mem_min + 3].int_ =
      font_info[fnt_infos.height_base[f] + (hd / 16)].int_;
  mem[b - mem_min + 2].int_ =
...
void tex::stack_into_box(halfword b, internal_font_number f, quarterword c) {
...
scaled tex::height_plus_depth(internal_font_number f, quarterword c) {
  four_quarters q = font_info[fnt_infos.char_base[f] + c].qqqq;
  uint8_t hd = q.b1;
...
halfword tex::var_delimiter(halfword d, small_number s, scaled v) {
  halfword b;
...
_L40:
  ...
  (mem[b - mem_min + 4].int_ =
       (half(mem[b - mem_min + 3].int_ - mem[b - mem_min + 2].int_) -
        (font_info[(fnt_infos.param_base[((eqtb[s + 10837].hh).rh)] + 22)]
             .int_)));
  return b;
}

halfword tex::rebox(halfword b, scaled w) {
  halfword p;
  internal_font_number f;
  scaled v;
...
halfword tex::math_glue(halfword g, scaled m) {
  halfword p;
...
void tex::math_kern(halfword p, scaled m) {
  integer n;
  scaled f;
  if (mem[p - mem_min].hh.U2.b1 != 99)
    return;
...
halfword tex::clean_box(halfword p, small_number s) {
  halfword q;
  ...
  cur_mu = x_over_n(
      font_info[fnt_infos.param_base[eqtb[cur_size + 10837].hh.rh] + 6].int_,
      18);
_L40:
...
void tex::fetch(halfword a) {
  ...
  if ((cur_c >= fnt_infos.font_bc[cur_f]) &&
      (cur_c <= fnt_infos.font_ec[cur_f]))
...
void tex::make_under(halfword q) {
  halfword y;
  scaled delta;
  ...
  delta =
      mem[y - mem_min + 3].int_ + mem[y - mem_min + 2].int_ +
      font_info[fnt_infos.param_base[eqtb[cur_size + 10838].hh.rh] + 8].int_;
...
void tex::make_vcenter(halfword q) {
  scaled delta;
...
void tex::make_radical(halfword q) {
  halfword y;
  scaled delta, clr;
  halfword x = clean_box(q + 1, ((cur_style / 2) * 2) + 1);
  if (cur_style < 2) {
    (clr =
         (font_info[fnt_infos.param_base[eqtb[cur_size + 10838].hh.rh] + 8]
              .int_ +
          (abs(font_info[fnt_infos.param_base[eqtb[cur_size + 10837].hh.rh] + 5]
                   .int_) /
           4)));
  } else {
    clr =
...
void tex::make_math_accent(halfword q) {
  halfword p, x, y;
  integer a;
  quarterword c;
  internal_font_number f;
    ...
    if ((cur_i.b2 & 3) == 1) {
      ...
      while (true) {
        if (cur_i.b1 == fnt_infos.skew_char[cur_f]) {
          if (cur_i.b2 >= 128) {
            if (cur_i.b0 <= 128)
              s = font_info[fnt_infos.kern_base[cur_f] + (cur_i.b2 * 256) +
                            cur_i.b3]
                      .int_;
          }
          goto _L31;
...
void tex::make_fraction(halfword q) {
  halfword p, v, x, y, z;
  ...
  if (cur_style < 2) {
    shift_up =
        font_info[fnt_infos.param_base[eqtb[cur_size + 10837].hh.rh] + 8].int_;
    shift_down =
    ...
    if (mem[q - mem_min + 1].int_)
      shift_up =
          font_info[fnt_infos.param_base[eqtb[cur_size + 10837].hh.rh] + 9]
              .int_;
    else
      shift_up =
          font_info[fnt_infos.param_base[eqtb[cur_size + 10837].hh.rh] + 10]
              .int_;
  }
  if (!mem[q - mem_min + 1].int_) {
    if (cur_style < 2)
      clr = font_info[fnt_infos.param_base[eqtb[cur_size + 10838].hh.rh] + 8]
                .int_ *
            7;
    else
      clr = font_info[fnt_infos.param_base[eqtb[cur_size + 10838].hh.rh] + 8]
    ...
    (delta1 =
         (clr - shift_up + mem[x - mem_min + 2].int_ +
  ...
  if (cur_style < 2)
    delta =
        font_info[fnt_infos.param_base[eqtb[cur_size + 10837].hh.rh] + 20].int_;
  else
    delta =
        font_info[fnt_infos.param_base[eqtb[cur_size + 10837].hh.rh] + 21].int_;
...
scaled tex::make_op(halfword q) {
  ...
  if (mem[q - mem_min + 1].hh.rh == 1) {
    ...
    (mem[x - mem_min + 4].int_ =
         (half(mem[x - mem_min + 3].int_ - mem[x - mem_min + 2].int_) -
          (font_info[(fnt_infos.param_base[((eqtb[cur_size + 10837].hh).rh)] +
                      22)]
               .int_)));
    mem[q - mem_min + 1].hh.rh = 2;
    mem[q - mem_min + 1].hh.lh = x;
  ...
  if (!mem[q - mem_min + 2].hh.rh) {
    ...
    shift_up =
        font_info[fnt_infos.param_base[eqtb[cur_size + 10838].hh.rh] + 11]
            .int_ -
        mem[x - mem_min + 2].int_;
    if (shift_up <
        font_info[fnt_infos.param_base[eqtb[cur_size + 10838].hh.rh] + 9].int_)
      shift_up =
          font_info[fnt_infos.param_base[eqtb[cur_size + 10838].hh.rh] + 9]
              .int_;
    p = new_kern(shift_up);
    mem[p - mem_min].hh.rh = y;
    mem[x - mem_min].hh.rh = p;
    p = new_kern(
        font_info[fnt_infos.param_base[eqtb[cur_size + 10838].hh.rh] + 13]
            .int_);
    mem[p - mem_min].hh.rh = x;
  ...
  if (!mem[q - mem_min + 3].hh.rh) {
    ...
    shift_down =
        font_info[fnt_infos.param_base[eqtb[cur_size + 10838].hh.rh] + 12]
            .int_ -
        mem[z - mem_min + 3].int_;
    if (shift_down <
        font_info[fnt_infos.param_base[eqtb[cur_size + 10838].hh.rh] + 10].int_)
      shift_down =
          font_info[fnt_infos.param_base[eqtb[cur_size + 10838].hh.rh] + 10]
              .int_;
    ...
    (mem[v - mem_min + 2].int_ +=
     (font_info[fnt_infos.param_base[eqtb[cur_size + 10838].hh.rh] + 13].int_ +
      mem[z - mem_min + 3].int_ + (mem[(z - mem_min + 2)].int_) + shift_down));
  }
  mem[q - mem_min + 1].int_ = v;
  return delta;
}

void tex::make_ord(halfword q) {
  integer a;
  halfword p, r;
_L20:
  if (!mem[q - mem_min + 3].hh.rh) {
            ...
            if (mem[p - mem_min + 1].hh.rh == 1) {
              if (mem[p - mem_min + 1].hh.U2.b0 ==
                ...
                if ((cur_i.b2 & 3) == 1) {
                  ...
                  cur_i = font_info[a].qqqq;
                  if (cur_i.b0 > 128) {
                    a = fnt_infos.lig_kern_base[cur_f] + (cur_i.b2 * 256) +
                        cur_i.b3;
                    cur_i = font_info[a].qqqq;
                  }
                  while (true) {
...
void tex::make_scripts(halfword q, scaled delta) {
  halfword x, y, z;
  ...
  if (!mem[q - mem_min + 2].hh.rh) {
    ...
    if (shift_down <
      ...
      shift_down =
          font_info[fnt_infos.param_base[eqtb[cur_size + 10837].hh.rh] + 16]
              .int_;
    ...
    mem[x - mem_min + 1].int_ += eqtb[12742].int_;
    if (cur_style & 1) {
      clr = font_info[fnt_infos.param_base[eqtb[cur_size + 10837].hh.rh] + 15]
                .int_;
    } else if (cur_style < 2)
      clr = font_info[fnt_infos.param_base[eqtb[cur_size + 10837].hh.rh] + 13]
                .int_;
    else
      clr = font_info[fnt_infos.param_base[eqtb[cur_size + 10837].hh.rh] + 14]
                .int_;
    if (shift_up < clr)
      shift_up = clr;
    ...
    if (!mem[q - mem_min + 3].hh.rh) {
      ...
      if (shift_down <
          font_info[fnt_infos.param_base[eqtb[cur_size + 10837].hh.rh] + 17]
              .int_)
        shift_down =
            font_info[fnt_infos.param_base[eqtb[cur_size + 10837].hh.rh] + 17]
                .int_;
      ...
      if (clr > 0) {
        ...
        (clr =
             ((abs(font_info
                       [fnt_infos.param_base[eqtb[cur_size + 10837].hh.rh] + 5]
                           .int_ *
                   4) /
               5) -
              shift_up + (mem[(x - mem_min + 2)].int_)));
...
small_number tex::make_left_right(halfword q, small_number style, scaled max_d,
                                  scaled max_h) {
  ...
  _L21:
    ...
    case 14:
      ...
      if (cur_style < 4)
        ...
        cur_size = ((cur_style - 2) / 2) * 16;
      cur_mu = x_over_n(
          font_info[fnt_infos.param_base[eqtb[cur_size + 10837].hh.rh] + 6]
              .int_,
          18);
      goto _L81;
      break;
...
void tex::fin_row() {
  ...
  if (cur_list.mode_field == (-102)) {
    ...
    if (cur_head != cur_tail) {
      mem[cur_list.tail_field - mem_min].hh.rh = mem[cur_head - mem_min].hh.rh;
...
halfword tex::finite_shrink(halfword p) {
  halfword q;
...
small_number tex::reconstitute(small_number j, small_number n, halfword bchar,
                               halfword hchar) {
...
quarterword tex::new_trie_op(small_number d, small_number n, quarterword v) {
...
trie_pointer tex::trie_node(trie_pointer p) {
...
trie_pointer tex::compress_trie(trie_pointer p) {
  if (!p) {
    return 0;
  } else {
...
void tex::first_fit(trie_pointer p) {
...
void tex::trie_pack(trie_pointer p) {
  trie_pointer q;
  do {
    q = trie_l[p];
    if ((q > 0) && (!trie_hash[q])) {
...
void tex::trie_fix(trie_pointer p) {
  trie_pointer q;
  ASCII_code c;
...
void tex::init_trie() {
  ...
  for (N = trie_op_ptr, j = 1; j <= N; ++j)
    trie_op_hash[j + trie_op_size] = op_start[trie_op_lang[j]] + trie_op_val[j];
  for (N = trie_op_ptr, j = 1; j <= N; ++j) {
  ...
  _L21:
    ...
    case 2:
      if (n > 1) {
        ...
        while (hyph_word[h]) {
          k = hyph_word[h];
          if (str_start[k + 1] - str_start[k] < str_start[s + 1] - str_start[s])
            goto _L40;
          if (str_start[k + 1] - str_start[k] > str_start[s + 1] - str_start[s])
            goto _L45;
          u = str_start[k];
...
halfword tex::prune_page_top(halfword p) {
...
halfword tex::vert_break(halfword p, scaled h, scaled d) {
  ...
  _L90:
    if (mem[p - mem_min].hh.U2.b0 == 11) {
      ...
      active_width[mem[q - mem_min].hh.U2.b0 + 2] += mem[q - mem_min + 2].int_;
      active_width[6] += mem[q - mem_min + 3].int_;
...
halfword tex::vsplit(uint8_t n, scaled h) {
  halfword result, p, q;
...
void tex::freeze_page_specs(small_number s) {
  page_contents = s;
...
void tex::ensure_vbox(uint8_t n) {
...
void tex::fire_up(halfword c) {
  ...
  eqtb[10833].hh.rh =
      vpackage(mem[mem_max - mem_min - 2].hh.rh, best_size, 0, page_max_depth);
...
void tex::app_space() {
  ...
  if ((cur_list.aux_field.hh.lh >= 2000) && eqtb[9795].hh.rh) {
    ...
    mem[main_p - mem_min + 2].int_ = xn_over_d(mem[main_p - mem_min + 2].int_,
                                               cur_list.aux_field.hh.lh, 1000);
    mem[main_p - mem_min + 3].int_ = xn_over_d(mem[main_p - mem_min + 3].int_,
                                               1000, cur_list.aux_field.hh.lh);
    q = new_glue(main_p);
...
void tex::box_end(integer box_context) {
  halfword p;
...
void tex::begin_box(integer box_context) {
  halfword p, q;
  quarterword m;
  halfword k;
  uint8_t n;
...
void tex::scan_box(integer box_context) {
  do {
    get_x_token();
  } while ((cur_cmd == 10) || (!cur_cmd));
  if (cur_cmd == 20) {
...
void tex::package(small_number c) {
  scaled h;
  halfword p;
...
small_number tex::norm_min(integer h) {
  if (h <= 0) {
    return 1;
  } else if (h >= 63)
    return 63;
...
void tex::append_italic_correction() {
  ...
  if (cur_list.tail_field != cur_list.head_field) {
    ...
    f = mem[p - mem_min].hh.U2.b0;
    (mem[cur_list.tail_field - mem_min].hh.rh =
         new_kern(font_info[fnt_infos.italic_base[f] +
                            (font_info[fnt_infos.char_base[f] +
                                       mem[p - mem_min].hh.U2.b1]
                                 .qqqq.b2 /
                             4)]
...
void tex::push_math(group_code c) {
  push_nest();
...
void tex::set_math_char(integer c) {
  halfword p;
  if (c >= 32768) {
    cur_cs = cur_chr + 1;
    cur_cmd = eqtb[cur_cs].hh.U2.b0;
...
halfword tex::fin_mlist(halfword p) {
  halfword q;
  if (cur_list.aux_field.int_ != (-1073741824)) {
    ...
    if (p == (-1073741824)) {
      ...
      if (mem[q - mem_min].hh.U2.b0 != 30)
        confusion(876);
      mem[cur_list.aux_field.int_ - mem_min + 2].hh.lh = mem[q - mem_min].hh.rh;
...
void tex::do_register_command(small_number a) {
  halfword l, r, s;
  char p;
  halfword q = cur_cmd;
  if (q != 89) {
...
void tex::new_font(small_number a) {
  halfword u;
  scaled s;
  internal_font_number f;
  str_number t;
    ...
    if (str_eq_str(fnt_infos.font_name[f], cur_name) &
      ...
      if (s > 0) {
        if (s == fnt_infos.font_size[f])
          goto _L50;
      } else if (fnt_infos.font_size[f] ==
                 xn_over_d(fnt_infos.font_dsize[f], -s, 1000))
        goto _L50;
    }
  }
...
void tex::store_fmt_file() {
  ...
  print_nl(338);
  slow_print(format_ident);
  readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = 117275187;
  writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
  readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = 0;
  writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
  readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = mem_max;
  writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
  ...
  writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
  readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = pool_ptr;
  writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
  readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = str_ptr;
  writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
  for (N = str_ptr, k = 0; k <= N; ++k) {
    readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = str_start[k];
    writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
  }
  k = 0;
  while (k + 4 < pool_ptr) {
  ...
  sort_avail();
  var_used = 0;
  readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = lo_mem_max;
  writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
  ...
  for (N = lo_mem_max, k = p; k <= N; ++k)
    fwrite((&mem[k - mem_min]), sizeof(memory_word), 1, fmt_file);
  x += lo_mem_max - p + 1;
  readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = hi_mem_min;
  writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
  ...
  readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = var_used;
  writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
  readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = dyn_used;
  writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
  print_ln();
  print_int(x);
  print(1260);
  ...
  _L31:
    readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = l - k;
    writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
    while (k < l) {
    ...
    k = j + 1;
    readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = k - l;
    writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
  } while (k != 12163);
  do {
    j = k;
  ...
  readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = par_loc;
  writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
  readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = write_loc;
  writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
  readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = hash_used;
  writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
  ...
  for (N1 = hash_used, p = 514; p <= N1; ++p) {
    if (hash[p - 514].rh) {
      ...
      writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
      readU32(fmt_file, fmt_file_mode, &fmt_file_value).hh = hash[p - 514];
      writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
      ++cs_count;
    }
  }
  ...
  readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = cs_count;
  writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
  print_ln();
  print_int(cs_count);
  print(1261);
  readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = fmem_ptr;
  writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
  for (N = fmem_ptr, k = 0; k < N; ++k)
    fwrite((&font_info[k]), sizeof(memory_word), 1, fmt_file);
  readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = font_ptr;
  writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
  ...
  if (font_ptr != 1)
    print_char(115);
  readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = hyph_count;
  writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
  for (k = 0; k <= 307; ++k) {
    if (hyph_word[k]) {
      readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = k;
      writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
      readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = hyph_word[k];
      writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
      readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = hyph_list[k];
      writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
    }
  }
  print_ln();
  ...
  if (trie_not_ready)
    init_trie();
  readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = trie_max;
  writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
  for (N = trie_max, k = 0; k <= N; ++k) {
    readU32(fmt_file, fmt_file_mode, &fmt_file_value).hh = trie[k];
    writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
  }
  readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = trie_op_ptr;
  writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
  for (N = trie_op_ptr, k = 1; k <= N; ++k) {
    readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = hyf_distance[k];
    writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
    readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = hyf_num[k];
    writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
    readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = hyf_next[k];
    writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
  }
  print_nl(1266);
  print_int(trie_max);
  ...
  for (k = 255; k >= 0; --k) {
    if (trie_used[k] > 0) {
      ...
      readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = k;
      writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
      readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = trie_used[k];
      writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
    }
  }
  readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = interaction;
  writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
  readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_ = format_ident;
  writeU32(fmt_file, fmt_file_mode, &fmt_file_value);
...
void tex::new_write_whatsit(small_number w) {
  ...
  case 11:
    ...
    if (save_stack[save_ptr].int_ < 255) {
      ...
      mem[cur_list.tail_field - mem_min + 3].int_ =
          mem[p - mem_min + 3].int_ + mem[p - mem_min + 2].int_;
      mem[cur_list.tail_field - mem_min + 4].hh.lh = mem[p - mem_min + 5].hh.rh;
      ...
      mem[cur_list.tail_field - mem_min].hh.U2.b1 = 0;
      mem[cur_list.tail_field - mem_min + 1].int_ = mem[p - mem_min + 5].hh.rh;
      delete_glue_ref(q);
    }
    free_node(p, 7);
    if (!nest_ptr)
...
_L91:
  if (lig_stack < hi_mem_min)
    goto _L95;
_L92:
  if ((cur_chr < fnt_infos.font_bc[main_f]) ||
      (cur_chr > fnt_infos.font_ec[main_f])) {
  ...
  if (main_j.b1 == cur_r) {
    if (main_j.b0 <= 128) {
      if (main_j.b2 >= 128) {
        ...
        (mem[cur_list.tail_field - mem_min].hh.rh =
             new_kern(font_info[fnt_infos.kern_base[main_f] +
                                (main_j.b2 * 256) + main_j.b3]
                          .int_));
        cur_list.tail_field = mem[cur_list.tail_field - mem_min].hh.rh;
        goto _L90;
      }
...
bool tex::load_fmt_file() {
  ...
  four_quarters w;
  int N;
  integer x = readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_;
  if (x != 117275187)
    goto _L6666;
  ...
  do {
    for (k = p; k <= (q + 1); ++k) {
      loadU32(fmt_file, fmt_file_mode, &fmt_file_value);
      mem[k - mem_min] = readU32(fmt_file, fmt_file_mode, &fmt_file_value);
    }
    p = q + mem[q - mem_min].hh.lh;
  ...
  loadU32(fmt_file, fmt_file_mode, &fmt_file_value);
  var_used = readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_;
  loadU32(fmt_file, fmt_file_mode, &fmt_file_value);
  dyn_used = readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_;
  k = 1;
  do {
    loadU32(fmt_file, fmt_file_mode, &fmt_file_value);
    x = readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_;
    ...
    for (j = k; j < (k + x); ++j) {
      loadU32(fmt_file, fmt_file_mode, &fmt_file_value);
      eqtb[j] = readU32(fmt_file, fmt_file_mode, &fmt_file_value);
    }
    k += x;
    loadU32(fmt_file, fmt_file_mode, &fmt_file_value);
    x = readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_;
  ...
  do {
    ...
    p = x;
    loadU32(fmt_file, fmt_file_mode, &fmt_file_value);
    hash[p - 514] = readU32(fmt_file, fmt_file_mode, &fmt_file_value).hh;
  } while (p != hash_used);
  for (p = hash_used + 1; p <= 9780; ++p) {
    loadU32(fmt_file, fmt_file_mode, &fmt_file_value);
    hash[p - 514] = readU32(fmt_file, fmt_file_mode, &fmt_file_value).hh;
  ...
  loadU32(fmt_file, fmt_file_mode, &fmt_file_value);
  cs_count = readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_;
  loadU32(fmt_file, fmt_file_mode, &fmt_file_value);
  x = readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_;
  if (x < 7)
    goto _L6666;
  ...
  for (N = fmem_ptr, k = 0; k < N; ++k) {
    loadU32(fmt_file, fmt_file_mode, &fmt_file_value);
    font_info[k] = readU32(fmt_file, fmt_file_mode, &fmt_file_value);
  }
  loadU32(fmt_file, fmt_file_mode, &fmt_file_value);
  x = readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_;
  if (x < 0)
  ...
  for (k = 0; k <= j; ++k) {
    loadU32(fmt_file, fmt_file_mode, &fmt_file_value);
    trie[k] = readU32(fmt_file, fmt_file_mode, &fmt_file_value).hh;
  }
  loadU32(fmt_file, fmt_file_mode, &fmt_file_value);
  x = readU32(fmt_file, fmt_file_mode, &fmt_file_value).int_;
  if (x < 0)
...
void tex::close_files_and_terminate() {
  integer k;
  for (k = 0; k <= 15; ++k) {
    if (write_open[k])
  ...
  if (!dvi_mgr.total_pages) {
    ...
    dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = 248;
    ++dvi_mgr.dvi_ptr;
    if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
      dvi_swap();
    dvi_four(last_bop);
    last_bop = dvi_mgr.dvi_offset + dvi_mgr.dvi_ptr - 5;
    dvi_four(25400000);
    dvi_four(473628672);
    prepare_mag();
    dvi_four(eqtb[12180].int_);
    dvi_four(max_v);
    dvi_four(max_h);
    dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = max_push / 256;
    ++dvi_mgr.dvi_ptr;
    if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
      dvi_swap();
    dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = max_push & 255;
    ++dvi_mgr.dvi_ptr;
    if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
      dvi_swap();
    dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = (dvi_mgr.total_pages / 256) & 255;
    ++dvi_mgr.dvi_ptr;
    if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
      dvi_swap();
    dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = dvi_mgr.total_pages & 255;
    ++dvi_mgr.dvi_ptr;
    if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
      dvi_swap();
    while (font_ptr > 0) {
    ...
    dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = 249;
    ++dvi_mgr.dvi_ptr;
    if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
      dvi_swap();
    dvi_four(last_bop);
    ...
    if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
      dvi_swap();
    k = ((dvi_buf_size - dvi_mgr.dvi_ptr) & 3) + 4;
    while (k > 0) {
      dvi_mgr.dvi_buf[dvi_mgr.dvi_ptr] = 223;
      ++dvi_mgr.dvi_ptr;
      if (dvi_mgr.dvi_ptr == dvi_mgr.dvi_limit)
        dvi_swap();
      --k;
    }
    if (dvi_mgr.dvi_limit == dvi_mgr.half_buf)
      write_dvi(dvi_mgr.half_buf, dvi_buf_size - 1);
    if (dvi_mgr.dvi_ptr > 0)
      write_dvi(0, dvi_mgr.dvi_ptr - 1);
    print_nl(837);
    slow_print(output_file_name);
    print(286);
    print_int(dvi_mgr.total_pages);
    print(838);
    if (dvi_mgr.total_pages != 1)
      print_char(115);
    print(839);
    print_int(dvi_mgr.dvi_offset + dvi_mgr.dvi_ptr);
    print(840);
